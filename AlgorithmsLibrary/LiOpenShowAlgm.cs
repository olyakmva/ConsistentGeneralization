...
namespace AlgorithmsLibrary
{
    public class LiOpenshawAlgm : ISimplificationAlgm
    {
        ...
        private void Run(List<MapPoint> vertices, int startIndx, ref int endIndx)
        {
            ...
            // Ли-Оупеншоу 
            MinMaxValuesCompute(new List<MapPoint>(vertices), out var xmin, out var ymin, out _, out _);
        ...
        private void MinMaxValuesCompute(List<MapPoint> mapPoints, out double xmin, out double ymin, out double xmax, out double ymax)
        {
            xmin = mapPoints.Min(point => point.X);
            xmax = mapPoints.Max(point => point.X);
            ymax = mapPoints.Max(point => point.Y);
            ymin = mapPoints.Min(point => point.Y);
        }
